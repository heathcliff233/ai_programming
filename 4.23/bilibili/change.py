...
"""
# https://api.bilibili.com/x/player/pagelist?aid=47441335
...
def fetch_sina_news():
    ...
    with open('CXK.txt ', 'w', encoding='utf-8') as f:
...
def extract_words():
    with open('CXK.txt ','r', encoding='utf-8') as f:
    ...
    mask_image = imread(path.join(d, "ikunkun.jpg"))
    wordcloud = WordCloud(font_path='simhei.ttf', background_color="white", mask=mask_image, max_words=200,scale=4,min_font_size = 6).generate_from_frequencies(content)
